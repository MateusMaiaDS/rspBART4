...
sp_nodeLogLike <- function(curr_part_res,
                        ancestors,
                        index_node,
                        data){
  ...
  if(length(D_subset_index)==0){
    ...
    cov_aux <- diag(x = (data$tau^(-1)),nrow = n_leaf)
...
grow <- function(tree,
  ...
  if(data$intercept_bool){
    ...
    g_loglike <- sp_nodeLogLike(curr_part_res = curr_part_res,
                             ancestors = unique(g_node$ancestors),
                             index_node = g_node$train_index,
                             data = data)


    left_loglike <-  sp_nodeLogLike(curr_part_res = curr_part_res,
                                 index_node = left_index,
                                 ancestors = unique(c(g_node$ancestors,p_var)),
                                 data = data)

    right_loglike <-  sp_nodeLogLike(curr_part_res = curr_part_res,
                                  index_node = right_index,
                                  ancestors = unique(c(g_node$ancestors,p_var)),
                                  data = data)
  }
...
prune <- function(tree,
  ...
  if(data$intercept_bool){
    ...
    p_left_loglike <-  sp_nodeLogLike(curr_part_res = curr_part_res,
                                   index_node = children_left_index,
                                   ancestors = unique(children_left_ancestors),
                                   data = data)
...
change <- function(tree,
  ...
  if(data$intercept_bool){
    ...
    c_loglike_left <- sp_nodeLogLike(curr_part_res = curr_part_res,
                                  index_node = tree[[c_node$left]]$train_index,
                                  ancestors = unique(tree[[c_node$left]]$ancestors),
                                  data = data)


    c_loglike_right <-  sp_nodeLogLike(curr_part_res = curr_part_res,
                                    index_node = tree[[c_node$right]]$train_index,
                                    ancestors = unique(tree[[c_node$right]]$ancestors),
                                    data = data)
  }
  ...
  if(data$intercept_bool){
    ...
    new_c_loglike_left <-  sp_nodeLogLike(curr_part_res = curr_part_res,
                                       index_node = left_index,
                                       ancestors = unique(new_left_ancestors),
                                       data = data)

    new_c_loglike_right <-  sp_nodeLogLike(curr_part_res = curr_part_res,
                                        index_node = right_index,
                                        ancestors = unique(new_right_ancestors),
                                        data = data)
  }
...
sp_updateBetas <- function(tree,
  ...
  for(i in 1:length(t_nodes_names)){
    ...
    if(length(cu_t$ancestors)==0){
      # There's nothing to update if there's no ancestors
      next
    }
...
sp_getPredictions <- function(tree,
                           data){
